...
def build_headline(symbol_dict):
    ...
    for symbol, name in symbol_dict.items():
        ...
        elif 'regularMarketLastPrice' in stock_info:
            headline += _color_yellow(name) + ' ' + _color_white(stock_info['regularMarketLastPrice']) + ' ' + _color_red_green(stock_info['regularMarketChangePercent']/100) + ' | '
    return headline
